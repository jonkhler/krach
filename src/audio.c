...
void reset_generator(struct signal_generator *gen) { gen->phase = -M_PI_M2; }
...
double process_sequence(struct sequence *seq)
{
    ...
    return process_envelope(seq->env) * process_signal_generator(seq->gen) *
           seq->notes[seq->pos]->vol;
}
